...
def main(input_filepath, output_filepath):
    ...
    train_images = (train_images - train_images.mean(axis=0)) / (
        train_images.std(axis=0) + 1e-9
    )
    train_labels = np.concatenate(train_labels)
    with open(output_filepath + "/corruptmnist_train.pkl", "wb") as fp:
        pickle.dump(
            (torch.from_numpy(train_images), torch.from_numpy(train_labels)), fp
        )
    test = np.load(input_filepath + "/test.npz", allow_pickle=True)
    test_images = (test["images"] - train_images.mean(axis=0)) / (
        train_images.std(axis=0) + 1e-9
    )
